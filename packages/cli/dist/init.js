...
function appendRule(filePath, label) {
    ...
    if (fs.existsSync(filePath)) {
        ...
        if (content.includes('PkgDiet')) {
            console.log(`  ⏭  ${label} already has PkgDiet rules — skipped`);
            return false;
        }
        fs.writeFileSync(filePath, content + '\n\n' + AI_RULE + '\n');
...
function writeMcpJson(filePath, label) {
    ...
    if (fs.existsSync(filePath)) {
        ...
        if (data?.mcpServers?.pkgdiet) {
            console.log(`  ⏭  ${label} already configured — skipped`);
            return false;
        }
    }
    if (!data.mcpServers)
...
export async function runInit(cwd, options = {}) {
    ...
    if (policy) {
        console.log('📋 Policy');
        if (writeJson(path.join(cwd, '.pkgdietrc.json'), DEFAULT_POLICY, '.pkgdietrc.json'))
            created++;
    }
    // 2. CI workflow
    if (ci) {
        console.log('\n🔁 GitHub Actions');
        if (writeText(path.join(cwd, '.github', 'workflows', 'pkgdiet.yml'), CI_WORKFLOW, '.github/workflows/pkgdiet.yml'))
            created++;
    }
    // 3. AI agents
    if (agents) {
        console.log('\n🤖 AI Agent configs');
        if (writeMcpJson(path.join(cwd, '.cursor', 'mcp.json'), 'Cursor (.cursor/mcp.json)'))
            created++;
        if (appendRule(path.join(cwd, '.cursorrules'), 'Cursor rules (.cursorrules)'))
            created++;
        if (appendRule(path.join(cwd, '.windsurfrules'), 'Windsurf rules (.windsurfrules)'))
            created++;
        if (writeMcpJson(path.join(cwd, 'cline_mcp_settings.json'), 'Cline (cline_mcp_settings.json)'))
            created++;
        if (writeMcpJson(path.join(cwd, '.github', 'mcp.json'), 'Copilot (.github/mcp.json)'))
            created++;
        if (appendRule(path.join(cwd, 'CLAUDE.md'), 'Claude Code (CLAUDE.md)'))
            created++;
        ...
        try {
            ...
            if (claudePath) {
                if (writeMcpJson(claudePath, 'Claude Desktop (global config)'))
                    created++;
            }
        }
        catch (_) { }
