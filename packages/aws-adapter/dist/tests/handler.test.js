...
function mockEvent(method, body, isBase64 = false) {
...
test('Missing request body', async () => {
    const res = await handler(mockEvent('POST')); // body undefined
...
test('tools/list returns JSON-RPC response with tools', async () => {
    ...
    assert.ok(body.result.tools.some((t) => t.name === 'check_dependency'));
});
