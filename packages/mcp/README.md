...
The official Model Context Protocol (MCP) server for PkgDiet. [![Glama MCP Server](https://glama.ai/mcp/servers/om-tajne/pkgdiet/badge)](https://glama.ai/mcp/servers/om-tajne/pkgdiet)
...
This package exposes PkgDiet's policy and dependency-evaluation logic as structured, AAA-rated MCP tools (such as check_dependency and suggest_alternative).

## Usage
Run the MCP server via the main CLI wrapper:
```bash
