...
function withToolTimeout(promise) {
    const timeout = new Promise(resolve => setTimeout(() => resolve({
        isError: true,
        content: [{ type: "text", text: JSON.stringify({ code: "TOOL_TIMEOUT", message: "Tool timed out — registry may be slow. Try again." }) }],
    }), TOOL_TIMEOUT_MS));
    ...
    server.tool("check_dependency", "Evaluate a single npm package against the active PkgDiet policy to determine if it is safe, lightweight, and healthy to install. Use this read-only tool before proposing or installing any package. It returns a structured verdict (ALLOW, WARN, BLOCK), health score, cost impact, and security signals. It does not install packages, edit files, or modify the project. Always evaluate a package with this tool before recommending it.", {
        ...
        return withToolTimeout((async () => {
            ...
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ code: "INVALID_INPUT", message: err instanceof Error ? err.message : String(err) }) }],
                };
            }
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ code: "EVALUATION_FAILED", message: err instanceof Error ? err.message : String(err) }, null, 2) }],
                };
            }
        })());
    });
    // ── check_dependencies ──────────────────────────────────────────────────────
    server.tool("check_dependencies", "Evaluate multiple npm packages in a single batch request against the active PkgDiet policy. Use this read-only tool before scaffolding a new project or proposing multiple packages at once. Limit: 50 packages per request. It does not install packages, modify lockfiles, or alter the workspace.", {
            ...
            try {
                ...
                const tasks = validNames.map(pkg => async () => {
                    try {
                        ...
                        return { packageName: pkg, verdict: "WARN", healthScore: null, reasons: [`Failed to evaluate: ${err instanceof Error ? err.message : String(err)}`] };
                    }
                });
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ code: "BATCH_EVALUATION_FAILED", message: err instanceof Error ? err.message : String(err) }) }],
                };
            }
        })());
    });
    ...
    server.tool("suggest_alternative", "Use this read-only tool after check_dependency returns WARN or BLOCK, or when a developer asks for a replacement for an npm package. Returns curated candidates with reasons, compatibility notes, and migration guidance. Verify each candidate with check_dependency before recommending or installing.", {
        ...
        return withToolTimeout((async () => {
            ...
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ error: { code: "INVALID_PACKAGE_NAME", message: err instanceof Error ? err.message : String(err) } }, null, 2) }],
                };
            }
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ error: { code: "SUGGEST_FAILED", message: err instanceof Error ? err.message : String(err) } }, null, 2) }],
                };
            }
        })());
    });
    // ── get_policy ──────────────────────────────────────────────────────────────
    server.tool("get_policy", "Retrieve the active PkgDiet dependency policy constraints for the current workspace. Use this read-only tool when interpreting a WARN or BLOCK verdict, or to understand the project's health and security rules before scaffolding. It does not modify the policy or any project files.", {
        environment: z.enum(["dev", "prod", "ci", "test"]).default("dev")
        ...
        return withToolTimeout((async () => {
            try {
                ...
                return {
                    isError: true,
                    content: [{ type: "text", text: JSON.stringify({ code: "POLICY_FAILED", message: err instanceof Error ? err.message : String(err) }, null, 2) }],
                };
            }
        })());
    });
