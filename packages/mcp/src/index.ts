...
export async function startMcpServer() {
  ...
  server.tool(
    "check_dependency",
    "Evaluate an npm package for health, cost, size, and policy compliance before install.",
    {
    ...
    async (args) => {
      ...
      try {
        ...
        const mappedResult = {
          ...
          alternatives: (result.alternatives || []).map((alt: any) => typeof alt === 'string' ? alt : (alt.replacement || alt.name || JSON.stringify(alt))),
        };
