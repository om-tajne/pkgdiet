...
export function createMcpServer() {
  ...
  server.tool(
    "suggest_alternative",
    "Suggest a lighter, healthier alternative for an npm package. Use this read-only tool when a developer asks for a package replacement, or when check_dependency returns a WARN or BLOCK verdict (e.g., due to bloat, security risks, or deprecation). It returns a curated list of alternative candidates, including compatibility notes, size guidance, and migration steps. Behavior: It queries the PkgDiet curated alternatives dataset for the given package name and returns ranked recommendations based on the specified runtime and reason. Guidelines: Always verify each recommended candidate with the check_dependency tool before actually installing it or presenting it as a final solution. Do not use this tool to install packages.",
    {
      packageName: z.string().min(1).max(214)
