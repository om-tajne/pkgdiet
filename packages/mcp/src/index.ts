...
export async function startMcpServer() {
  ...
  server.tool(
    "suggest_alternative",
    "Suggest lighter, modern, or more secure alternatives to a given npm package. Call this when you want to replace deprecated or bloated dependencies.",
    {
    ...
    async (args) => {
      try {
        ...
        const recommendations = (altData.details || []).slice(0, limit).map((a: any) => ({
          name: a.name,
