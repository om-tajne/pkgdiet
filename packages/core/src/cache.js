...
function loadCache(projectPath) {
  ...
  try {
    ...
    try { renameSync(cachePath, corruptPath); } catch { /* ignore rename failure */ }
    return { version: 1, entries: {} };
  }

  if (!cache || cache.version !== 1 || typeof cache.entries !== 'object') {
    const corruptPath = `${cachePath}.corrupt.${Date.now()}`;
    try { renameSync(cachePath, corruptPath); } catch { /* ignore */ }
    return { version: 1, entries: {} };
  }
...
function evictOldest(cache) {
  ...
  for (const key of toRemove) delete newEntries[key];
