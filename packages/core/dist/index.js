...
function computeScores(unusedCount, unhealthyCount, criticalCount) {
    const overallScore = Math.max(0, 100 - unusedCount * 5 - unhealthyCount * 10 - criticalCount * 5);
    const repoSafetyScore = Math.max(0, 100 - criticalCount * 15);
...
export async function run(options = {}) {
    ...
    const { overallScore, repoSafetyScore } = computeScores(unusedDependencies.length, unhealthyDependencies.length, criticalCount);
    return {
        projectName,
