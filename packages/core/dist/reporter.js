...
export function renderReport(results, options = {}) {
    ...
    if (showUnused) {
        if (scanResult.unused.length > 0) {
            ...
            output.push(sectionHeader('🗑️', 'UNUSED DEPENDENCIES', `${scanResult.unused.length} found${unusedSize ? ` — saves ~${formatBytes(unusedSize)}` : ''}`));
            output.push('');
            ...
            for (const pkgName of scanResult.unused) {
                ...
                output.push(`   ${chalk.red('⚫')} ${pad(chalk.white(pkgName), 24)} ${pad(type, 8)} ${pad(size, 12)} ${chalk.gray(`→ npm uninstall ${pkgName}`)}`);
            }
    ...
    if (showHealth && healthResults) {
        ...
        if (issues.length > 0) {
            ...
            output.push('');
            output.push(`   ${chalk.gray(pad('Package', 26))} ${chalk.gray(pad('Score', 8))} ${chalk.gray('Issue')}`);
            ...
            for (const pkg of sorted) {
                ...
                output.push(`   ${icon} ${pad(chalk.white(pkg.name), 24)} ${pad(scoreColor(scoreStr), 8)} ${chalk.gray(joinedFlags)}`);
            }
        }
        else {
    ...
    if (showSize && sizeResult) {
        if (sizeResult.unsupported) {
            ...
            if (actionable.length > 0) {
                ...
                output.push('');
                output.push(`   ${chalk.gray(pad('Package', 28))} ${chalk.gray(pad('Install Size', 15))} ${chalk.gray('% of node_modules')}`);
                ...
                for (const pkg of actionable) {
                    ...
                    output.push(`   ${sizeIcon} ${pad(chalk.white(pkg.name), 26)} ${pad(formatBytes(pkg.size), 15)} ${chalk.gray(pctStr)}`);
                }
    ...
    const actions = [];
    if (removeCount > 0)
        actions.push(`${removeCount} to remove`);
    if (investigateCount > 0)
        actions.push(`${investigateCount} to investigate`);
    if (swapCount > 0)
        actions.push(`${swapCount} to swap`);
    if (actions.length > 0) {
