...
http.post("/github-webhook", async (c) => {
    ...
    try {
        await webhooks.verifyAndReceive({ id, name: event, payload: JSON.parse(body), signature });
        return c.text("OK");
    }
    catch (err) {
...
// Core PR handler
// ──────────────────────────────────────────────────────────────
async function handlePullRequest(payload, _octokit, prLog = log) {
...
// ──────────────────────────────────────────────────────────────
function formatPrComment(results, lockfileType) {
    const rows = results
        .map((r) => {
        ...
        const cost = r.evalResult.costEstimate
            ...
            : "0.000";
        return (`| \`${r.dep.name}\` | ${icon} ${r.evalResult.verdict} ` +
...
serve({ fetch: http.fetch, port: Number(process.env.PORT ?? 3000) }, (info) => {
